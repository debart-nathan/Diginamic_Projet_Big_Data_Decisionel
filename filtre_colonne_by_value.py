...
def drop_value_colonne(df, colonne, value, verbose = True): 
    ...
    if(verbose):
        ...
        print(f"Nombre de dates inférieurs à 2004 avant transformation : {nb_before}")
        print()

        print(f"Nombre de dates inférieurs à 2004 après traitement : {nb_after}")
        print()

        # Nombre de colonnes et de lignes total après traitement
        print(f"Nombre de ligne  dans de le DataFrame après transformation de la colonne datcde : {df_drop.shape[0]}")
        print()
